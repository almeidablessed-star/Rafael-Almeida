...
export const CatalogModule: React.FC<CatalogModuleProps> = ({
  ...
  return (
    <div className="space-y-5 pb-12">
      {/* Header Badge */}
      <div className="bg-gradient-to-r from-[#8E5CF0] via-[#E5613C] to-[#E9B839] rounded-lg p-4 text-white shadow-sm">
      ...
      {/* Navigation Sub-Tabs */}
      <div className="flex p-1 bg-[var(--color-neutral-off-white)] border border-[var(--color-pastry-light-pink)]/30 rounded-xl">
        <button
      ...
      {addedSuccessMsg && (
        <div className="bg-[var(--color-primary)]/10 border border-[var(--color-primary)] text-[var(--color-primary)] text-xs p-3.5 rounded-xl flex items-center gap-2.5 animate-fadeIn">
          ...
          <div className="bg-white p-4 rounded-lg border border-[#E6E1DB]/80 shadow-sm space-y-3">
            ...
            <div className="pt-2.5 border-t border-[#E6E1DB]/80 space-y-2">
              <div className="flex items-center justify-between">
                ...
                <div className="flex bg-[#E6E1DB] p-0.5 rounded-xl border border-[#E6E1DB]">
                  <button
                    type="button"
                    onClick={() => {
                ...
                <div className="bg-[var(--color-accent-gold)]/20/70 p-3 rounded-xl border border-[var(--color-accent-gold)]/30 space-y-2 animate-fadeIn">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="text-right shrink-0 pt-3">
                      <span className="text-[10px] text-[#E6E1DB] block font-bold">Taxa ($1.50/mi)</span>
                      <span className="text-sm font-black text-[var(--color-accent-gold)]">
            ...
            <div className="pt-2.5 border-t border-[#E6E1DB]/80 space-y-2">
              ...
              {hasAddons && (
                <div className="bg-[#B8D4E8]/20/70 p-3 rounded-xl border border-[#B8D4E8]/30 space-y-3 animate-fadeIn">
                      ...
                      <div key={addon.id} className="bg-white p-3 rounded-xl border border-[#B8D4E8]/30/80 shadow-xs space-y-2.5">
                        <div className="flex items-center justify-between">
                          ...
                          {addons.length > 1 && (
                            <button
                              ...
                              className="p-1 text-[#E6E1DB] hover:text-[#C85A54] transition rounded-lg hover:bg-[#C85A54]/10"
                              title="Remover item"
                            >
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          ...
                          <div>
                            ...
                            <div className="relative">
                              <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[#E6E1DB] font-bold text-xs">
                                $
                              </span>
                              <input
                                type="number"
                        ...
                        <div className="pt-1 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2 text-[11px]">
                          <label className="flex items-center gap-1.5 cursor-pointer text-[#0D0B08] hover:text-[#0D0B08] font-medium select-none">
                            <input
                              ...
                              className="rounded text-[#B8D4E8] focus:ring-semantic-info w-3.5 h-3.5"
                            />
                          ...
                          {addon.hasCost && (
                            <div className="flex items-center gap-1.5 animate-fadeIn">
                              <span className="text-[#E6E1DB] font-bold">Custo ($):</span>
                              <input
                                type="number"
                                min="0"
                                step="0.5"
                                ...
                                placeholder="0.00"
                                className="w-20 px-2 py-1 bg-semantic-warning/20 border border-semantic-warning rounded text-xs font-bold text-semantic-warning focus:outline-none focus:ring-1 focus:ring-semantic-warning"
                              />
                            </div>
                          )}
                        </div>
                  ...
                  {totalAddonsValue > 0 && (
                    <div className="flex items-center justify-between bg-[#B8D4E8]/20/80 p-2 rounded-xl border border-[#B8D4E8]/30 text-xs text-[#B8D4E8] font-bold">
          ...
          <div className="bg-white rounded-lg border border-[var(--color-accent-gold)]/30 shadow-sm p-4 space-y-4 relative overflow-hidden">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              ...
              <div className="text-right">
                <span className="text-[10px] text-[#E6E1DB] block font-medium">Preço Total de Venda</span>
                ...
                {(hasDelivery && deliveryFee > 0 || totalAddonsValue > 0) && (
                  <span className="text-[10px] font-bold text-[var(--color-accent-gold)] block">
            ...
            <div className="grid grid-cols-2 gap-2.5">
              <div className="p-2.5 bg-semantic-warning/20/70 rounded-xl border border-amber-100/80">
                <div className="flex items-center justify-between text-semantic-warning text-xs font-bold">
                  <span>🔄 Reposição</span>
                  <span>{formatCurrency(currentRecipe.reposicao)}</span>
                </div>
                <p className="text-[10px] text-semantic-warning/80 mt-0.5">Ingredientes / Insumos</p>
              </div>

              <div className="p-2.5 bg-[#B8D4E8]/20/70 rounded-xl border border-[#B8D4E8]/30/80">
                <div className="flex items-center justify-between text-[#B8D4E8] text-xs font-bold">
                  <span>🛠️ Mão de Obra</span>
                  <span>{formatCurrency(currentRecipe.maodeobra)}</span>
                </div>
                <p className="text-[10px] text-[#B8D4E8]/80 mt-0.5">Seu Salário</p>
              </div>

              <div className="p-2.5 bg-[#C85A54]/20/70 rounded-xl border border-semantic-error/30/80">
                <div className="flex items-center justify-between text-[#C85A54] text-xs font-bold">
                  <span>📦 Custo Op.</span>
                  <span>{formatCurrency(currentRecipe.custo)}</span>
                </div>
                <p className="text-[10px] text-[#C85A54]/80 mt-0.5">Gás, Luz, Embalagem</p>
              </div>
              ...
              <div className="p-2.5 bg-[#B8D4E8]/20/70 rounded-xl border border-[#B8D4E8]/30/80">
                ...
                <p className="text-[10px] text-[#B8D4E8]/80 mt-0.5">Caixa de Crescimento</p>
              </div>

              {/* Delivery Fee Line */}
              {hasDelivery && deliveryFee > 0 && (
                <div className="col-span-2 p-2.5 bg-[var(--color-accent-gold)]/20 rounded-xl border border-pink-200 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="text-base">🚗</span>
                    <div>
                      <span className="text-xs font-bold text-[var(--color-accent-gold)] block">
                        Taxa de Entrega ({numericMiles} {numericMiles === 1 ? 'milha' : 'milhas'})
                      </span>
                      <span className="text-[10px] text-[var(--color-accent-gold)] font-medium">
              ...
              {hasAddons && activeAddons.map((addon, idx) => {
                ...
                return (
                  <div key={addon.id} className="col-span-2 p-2.5 bg-[#B8D4E8]/20 rounded-xl border border-[#B8D4E8]/30 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="text-base">🌸</span>
                      <div>
                        <span className="text-xs font-bold text-[#B8D4E8] block">
                          🌸 Adicional ({descText})
                        </span>
                        <span className="text-[10px] text-[#B8D4E8] font-medium">
                    ...
                    <span className="text-sm font-black text-[#B8D4E8]">
                      +{formatCurrency(aVal)}
                    </span>
                  </div>
                );
            ...
            {/* Net Profit Callout */}
            {(hasDelivery && deliveryFee > 0 || totalAddonsValue > 0) && (
              <div className="p-2.5 bg-[var(--color-primary)]/10 rounded-xl border border-[var(--color-primary)]/40 text-xs text-[var(--color-primary)] font-medium flex items-center justify-between">
                <span>
            ...
            <button
              ...
              className="w-full py-3 bg-[var(--color-primary)] hover:brightness-110 text-white font-bold text-sm rounded-xl shadow-sm active:scale-98 transition-all flex items-center justify-center gap-2"
            >
      ...
      {activeSubTab === 'fechamento' && (
        <div className="space-y-4">
          <div className="bg-white p-4 rounded-lg border border-[#E6E1DB]/80 shadow-sm space-y-3">
            ...
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#E6E1DB] font-bold text-sm">
                $
              </span>
              <input
                type="number"
          ...
          <div className="bg-white rounded-lg border border-[#E6E1DB]/80 shadow-sm p-4 space-y-3">
            ...
            <div className="space-y-2.5 text-xs font-semibold">
              <div className="flex items-center justify-between p-2.5 bg-semantic-warning/20 text-semantic-warning rounded-xl border border-amber-100">
                <div className="flex items-center gap-2">
                  <span className="text-base">🔄</span>
                  <div>
                    <span className="font-bold block">1. Reposição de Insumos (30%)</span>
                    <span className="text-[10px] text-semantic-warning">Comprar ingredientes para o próximo mês</span>
                  </div>
                </div>
                <span className="text-sm font-extrabold">{formatCurrency(monthlyBreakdown.reposicao)}</span>
              </div>

              <div className="flex items-center justify-between p-2.5 bg-[#B8D4E8]/20 text-[#B8D4E8] rounded-xl border border-[#B8D4E8]/30">
                <div className="flex items-center gap-2">
                  <span className="text-base">🛠️</span>
                  <div>
                    <span className="font-bold block">2. Sua Mão de Obra / Salário (33,3%)</span>
                    <span className="text-[10px] text-[#B8D4E8]">Seu dinheiro limpo para contas pessoais</span>
                  </div>
                </div>
                <span className="text-sm font-extrabold">{formatCurrency(monthlyBreakdown.maodeobra)}</span>
              </div>

              <div className="flex items-center justify-between p-2.5 bg-[#C85A54]/20 text-[#C85A54] rounded-xl border border-semantic-error/30">
                <div className="flex items-center gap-2">
                  <span className="text-base">📦</span>
                  <div>
                    <span className="font-bold block">3. Custos Operacionais (16,7%)</span>
                    <span className="text-[10px] text-[#C85A54]">Gás, energia, água, embalagens</span>
                  </div>
                </div>
              ...
              <div className="flex items-center justify-between p-2.5 bg-[#B8D4E8]/20 text-[#B8D4E8] rounded-xl border border-[#B8D4E8]/30">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <span className="text-[10px] text-[#B8D4E8]">Comprar bicos, formas, cursos e reserva</span>
                  </div>
                </div>
      ...
      {activeSubTab === 'duvidas' && (
        <div className="space-y-4">
          <div className="bg-white p-4 rounded-lg border border-[#E6E1DB]/80 shadow-sm space-y-3">
            ...
            <div className="grid grid-cols-1 gap-2">
              <button
                ...
                className="p-2.5 bg-[#B8D4E8]/20 hover:bg-[#B8D4E8]/20 text-[#B8D4E8] rounded-xl text-left text-xs font-bold transition flex items-center justify-between"
              >
                <span>🎂 Bolo Franciele 20 cm (Divisão de custos)</span>
                <ArrowRight className="w-4 h-4 text-[#B8D4E8] shrink-0" />
              </button>

              <button
                onClick={() => handleQuickQuestion('Faturei $ 6,000 no mês. Como fica meu fechamento?')}
                className="p-2.5 bg-[#C85A54]/20 hover:bg-[#C85A54]/30 text-[#C85A54] rounded-xl text-left text-xs font-bold transition flex items-center justify-between"
              >
                <span>📊 Fechamento Mensal de $ 6,000.00</span>
                <ArrowRight className="w-4 h-4 text-[#C85A54] shrink-0" />
              </button>

              <button
                onClick={() => handleQuickQuestion('Vendi $ 4,500 no mês. Quanto posso tirar de salário?')}
                className="p-2.5 bg-[var(--color-primary)]/10 hover:bg-[var(--color-primary)]/20 text-[var(--color-primary)] rounded-xl text-left text-xs font-bold transition flex items-center justify-between"
              >
            ...
            <div className="pt-2">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 bg-neutral-hero border border-[#E6E1DB] rounded-xl text-xs font-medium focus:outline-none focus:ring-2 focus:ring-semantic-info"
                />
                <button
          ...
          {/* Answer Box */}
          {assistantAnswer && (
            <div className="bg-[var(--color-neutral-cream)] border border-[var(--color-primary)]/30 text-[var(--color-neutral-charcoal)] p-4 rounded-lg shadow-sm space-y-2 animate-fadeIn">
              <div className="flex items-center gap-1.5 text-[var(--color-primary)] text-xs font-bold uppercase tracking-wide">
                <Sparkles className="w-4 h-4 text-[var(--color-primary)]" />
                Resposta do Assistente
              </div>
              <div className="text-xs leading-relaxed whitespace-pre-line text-[var(--color-neutral-dark-gray)]">
                {assistantAnswer}
              </div>
            </div>
          )}
