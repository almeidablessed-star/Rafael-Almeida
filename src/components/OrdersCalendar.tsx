      ...
      <div>
        ...
        <div className="grid grid-cols-7 gap-1.5">
          ...
          {Array.from({ length: startingDayOfWeek }).map((_, i) => (
            <div key={`empty-${i}`} className="aspect-square rounded-2xl flex items-center justify-center font-black text-sm" style={{ color: '#D3C9D6', background: 'transparent' }} />
          ))}
          ...
          {Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            return (
              <button
                key={dayStr}
                className="aspect-square rounded-2xl flex items-center justify-center font-black text-sm transition-all hover:-translate-y-0.5 cursor-pointer"
                style={{
                  background: bgColor,
                  color: textColor,
                  border: borderStyle,
