...
export const CustomersModule: React.FC = () => {
  ...
  return (
    <div className="space-y-5 pb-12 animate-fadeIn">
      {/* Top Title Header */}
      <div className="flex items-center justify-between gap-2 border-b border-[#E8A0B0]/40 pb-3">
        <div className="flex items-center gap-2.5">
          <div className="p-2.5 bg-[#F5C6CE] text-[#2B2420] rounded-2xl shadow-2xs">
            <Users className="w-5 h-5 text-[#2B2420]" />
          </div>
          <div>
            <h2 className="font-editorial-bold text-xl sm:text-2xl text-[#2B2420]">
              Clientes
            </h2>
      ...
      {/* Success Toast */}
      {showToast && (
        <div className="bg-[#D6E4CC] text-[#2B2420] p-3.5 rounded-2xl shadow-sm text-xs font-bold flex items-center gap-2 border border-[#2B2420]/10 animate-bounce">
          <CheckCircle2 className="w-4 h-4 text-[#2B2420] shrink-0" />
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5">
        <div className="relative flex-1">
          <Search className="w-4 h-4 text-[#2B2420]/40 absolute left-3.5 top-1/2 -translate-y-1/2" />
          <input
            type="text"
            placeholder="Buscar cliente por nome, telefone ou cidade..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full pl-10 pr-3.5 py-2.5 bg-white border border-[#E8A0B0]/50 rounded-2xl text-xs font-bold text-[#2B2420] focus:outline-none focus:ring-2 focus:ring-[#E8A0B0] shadow-2xs"
          />
        </div>

        <button
          onClick={handleOpenAdd}
          className="py-2.5 px-4 bg-[#2B2420] hover:bg-black text-[#F5C6CE] font-bold text-xs rounded-2xl shadow-xs active:scale-95 transition-all flex items-center justify-center gap-1.5 shrink-0 cursor-pointer"
        >
      ...
      {isFormOpen && (
        <form
          onSubmit={handleSave}
          className="bg-white p-5 rounded-3xl border-2 border-[#E8A0B0] shadow-md space-y-4 animate-fadeIn"
        >
          <div className="flex items-center justify-between pb-3 border-b border-[#F8F1E4]">
            <h3 className="font-brand font-black text-base text-[#2B2420] flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-[#E8A0B0]" />
              {editingId ? 'Editar Cadastro da Cliente' : 'Cadastrar Nova Cliente'}
            </h3>
            <button
              type="button"
              onClick={() => setIsFormOpen(false)}
              className="text-xs text-[#2B2420]/60 hover:text-[#2B2420] font-bold px-2 py-1"
            >
              Cancelar
            </button>
          </div>
          ...
          <div className="flex items-center gap-4 bg-[#F8F1E4] p-3 rounded-2xl border border-[#E8A0B0]/40">
            <div className="w-16 h-16 rounded-full border-2 border-[#E8A0B0] bg-white text-[#2B2420] flex items-center justify-center overflow-hidden shrink-0 shadow-2xs relative">
              {photoUrl ? (
                <img src={photoUrl} alt="Preview" className="w-full h-full object-cover" />
              ) : (
                <User className="w-8 h-8 text-[#E8A0B0]" />
              )}
            </div>
            <div>
              <label htmlFor="photo-upload" className="block text-xs font-black text-[#2B2420] mb-1 flex items-center gap-1 cursor-pointer">
                <Camera className="w-3.5 h-3.5 text-[#E8A0B0]" /> Foto da Cliente
              </label>
              <label
                htmlFor="photo-upload"
                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#2B2420] text-[#F5C6CE] hover:bg-black rounded-xl text-xs font-bold cursor-pointer transition-colors shadow-2xs"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
            <div>
              <label className="block text-xs font-black text-[#2B2420] mb-1 flex items-center gap-1">
                👤 Nome da Cliente *
              </label>
              <input
                type="text"
                ...
                className="w-full px-3.5 py-2 bg-[#F8F1E4]/50 border border-[#E8A0B0]/40 rounded-xl text-xs font-bold text-[#2B2420] focus:outline-none focus:ring-2 focus:ring-[#E8A0B0]"
              />
            </div>

            <div>
              <label className="block text-xs font-black text-[#2B2420] mb-1 flex items-center gap-1">
                <Phone className="w-3.5 h-3.5 text-[#E8A0B0]" /> Telefone / WhatsApp *
              </label>
              <input
                type="text"
                required
            ...
            <div className="sm:col-span-2">
              <label className="block text-xs font-black text-[#2B2420] mb-1 flex items-center gap-1">
                <Calendar className="w-3.5 h-3.5 text-[#2B2420]" /> Data do Aniversário da Cliente
              </label>
              <input
                type="date"
                value={eventDate}
          ...
          <div className="space-y-2 pt-2 border-t border-[#F8F1E4]">
            <div className="flex items-center justify-between">
              <label className="text-xs font-black text-[#2B2420] flex items-center gap-1">
                <PlusCircle className="w-3.5 h-3.5 text-[#E8A0B0]" />
                Datas Comemorativas
              </label>
              <button
                type="button"
                onClick={handleAddExtraEventField}
                className="px-2.5 py-1 bg-[#D8CDEB] hover:bg-[#D8CDEB]/80 text-[#2B2420] rounded-lg text-[10px] font-extrabold flex items-center gap-1 cursor-pointer"
              >
            ...
            {additionalEvents.map((ev) => (
              <div key={ev.id} className="flex items-center gap-2 bg-[#F8F1E4] p-2 rounded-xl border border-[#E8A0B0]/40">
                <input
                  type="text"
                  placeholder="Nome da data comemorativa"
                  value={ev.title}
                  onChange={(e) => handleUpdateExtraEvent(ev.id, 'title', e.target.value)}
                  className="flex-1 px-3 py-1.5 bg-white border border-[#E8A0B0]/40 rounded-lg text-xs font-bold text-[#2B2420]"
                />
                <input
                  type="date"
                  value={ev.date}
                  onChange={(e) => handleUpdateExtraEvent(ev.id, 'date', e.target.value)}
                  className="px-3 py-1.5 bg-white border border-[#E8A0B0]/40 rounded-lg text-xs font-bold text-[#2B2420]"
                />
                <button
                  type="button"
                  onClick={() => handleRemoveExtraEvent(ev.id)}
                  className="p-1 text-[#2B2420]/40 hover:text-rose-600 cursor-pointer"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
            <div>
              <label className="block text-xs font-black text-[#2B2420] mb-1 flex items-center gap-1">
                <MapPin className="w-3.5 h-3.5 text-[#E8A0B0]" /> Endereço de Entrega
              </label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-xs font-black text-[#2B2420] mb-1 flex items-center gap-1">
                <Building className="w-3.5 h-3.5 text-[#2B2420]" /> Cidade
              </label>
              <input
                type="text"
          ...
          <div>
            <label className="block text-xs font-black text-[#2B2420] mb-1 flex items-center gap-1">
              <FileText className="w-3.5 h-3.5 text-[#2B2420]/60" /> Observações
            </label>
            <textarea
              rows={2}
              ...
              className="w-full px-3.5 py-2 bg-[#F8F1E4]/50 border border-[#E8A0B0]/40 rounded-xl text-xs font-medium text-[#2B2420] focus:outline-none focus:ring-2 focus:ring-[#E8A0B0]"
            />
          </div>

          <div className="flex justify-end gap-2 pt-2 border-t border-[#F8F1E4]">
            <button
              type="button"
              onClick={() => setIsFormOpen(false)}
              className="px-4 py-2 rounded-xl bg-[#F8F1E4] text-[#2B2420] font-bold text-xs hover:bg-[#F5C6CE]/20"
            >
              Cancelar
            </button>
            <button
              type="submit"
              className="px-5 py-2 rounded-xl bg-[#2B2420] hover:bg-black text-[#F5C6CE] font-brand font-bold text-xs shadow-md"
            >
              Salvar Cadastro
            </button>
          </div>
      ...
      <div className="space-y-3">
        {filteredCustomers.length === 0 ? (
          <div className="p-8 rounded-3xl bg-white border border-dashed border-[#E8A0B0]/40 text-center space-y-2">
            <Users className="w-10 h-10 text-[#2B2420]/30 mx-auto" />
            <p className="text-xs text-[#2B2420]/60 font-medium">
          ...
          filteredCustomers.map((c) => {
            ...
            return (
              <div
                key={c.id}
                className="p-4 rounded-3xl border border-[#E8A0B0]/40 bg-white shadow-2xs hover:border-[#E8A0B0] transition-all space-y-2.5"
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="space-y-1 min-w-0 flex-1">
                    <div className="flex items-center gap-2.5 flex-wrap">
                      {/* Customer Photo Avatar */}
                      <div className="w-9 h-9 rounded-full border-2 border-[#E8A0B0] bg-[#F5C6CE] text-[#2B2420] flex items-center justify-center overflow-hidden shrink-0 shadow-2xs">
                        {c.photoUrl ? (
                          <img src={c.photoUrl} alt={c.name} className="w-full h-full object-cover" />
                        ) : (
                          <User className="w-4 h-4 text-[#2B2420]" />
                        )}
                      </div>

                      <span className="font-editorial-bold text-base text-[#2B2420]">
                        {c.name}
                      </span>

                      {c.city && (
                        <span className="bg-[#D8CDEB] text-[#2B2420] text-[10px] font-bold px-2 py-0.5 rounded-full">
                          📍 {c.city}
                        </span>
                      )}
                    </div>

                    <div className="flex items-center gap-3 text-xs text-[#2B2420]/80 flex-wrap">
                      <span className="font-bold flex items-center gap-1 text-[#2B2420]">
                        📞 {c.phone}
                      </span>
                      {c.address && (
                        <span className="text-[#2B2420]/60 truncate">
                          🏠 {c.address}
                        </span>
                      )}
                    </div>
                    ...
                    {c.notes && (
                      <p className="text-[11px] text-[#2B2420]/70 italic bg-[#F8F1E4] p-2 rounded-xl border border-[#E8A0B0]/30">
                        📝 "{c.notes}"
                      </p>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <button
                      ...
                      className="p-2 rounded-xl bg-[#2B2420] text-[#F5C6CE] hover:bg-black font-bold text-xs flex items-center gap-1 transition-colors cursor-pointer"
                      title="Mandar mensagem no WhatsApp sem ano e sem link"
                    >
                      <MessageCircle className="w-4 h-4 text-[#F5C6CE]" />
                    </button>
                    <button
                      onClick={() => handleOpenEdit(c)}
                      className="p-2 rounded-xl text-[#2B2420]/60 hover:text-[#2B2420] hover:bg-[#F8F1E4] transition-colors cursor-pointer"
                    ...
                    <button
                      ...
                      className="p-2 rounded-xl text-[#2B2420]/40 hover:text-rose-600 hover:bg-rose-50 transition-colors cursor-pointer"
                ...
                <div className="pt-2 border-t border-[#E8A0B0]/30 mt-1">
                  <button
                    type="button"
                    onClick={() => setExpandedDatesCustomerId(expandedDatesCustomerId === c.id ? null : c.id)}
                    className="w-full py-2 px-3 rounded-2xl bg-[#F8F1E4] hover:bg-[#F5C6CE]/30 text-[#2B2420] text-xs font-bold flex items-center justify-between transition-all cursor-pointer border border-[#E8A0B0]/40 shadow-2xs"
                  >
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className="text-[10px] font-black bg-[#2B2420] text-[#F5C6CE] px-2.5 py-0.5 rounded-full">
                        {cEvents.length} {cEvents.length === 1 ? 'data' : 'datas'}
                      </span>
                      {expandedDatesCustomerId === c.id ? (
                        <ChevronUp className="w-4 h-4 text-[#2B2420]" />
                      ) : (
                        <ChevronDown className="w-4 h-4 text-[#2B2420]" />
                      )}
                    </div>
                  </button>
                  ...
                  {expandedDatesCustomerId === c.id && (
                    <div className="mt-2.5 space-y-2 p-3 bg-[#F8F1E4]/60 rounded-2xl border border-[#E8A0B0]/40 animate-fadeIn">
                      <p className="text-[10px] font-extrabold uppercase tracking-wider text-[#2B2420]/80">
                        ...
                        {cEvents.map((ev, idx) => (
                          <div
                            ...
                            className={`p-2.5 rounded-xl border flex items-center justify-between gap-2 text-xs ${
                              ev.isHoliday
                                ? 'bg-[#D8CDEB]/30 border-[#D8CDEB]'
                                : 'bg-white border-[#E8A0B0]/50 shadow-2xs'
                            }`}
                          >
                            <div className="min-w-0 flex-1">
                              <div className="flex items-center gap-1">
                                <span className="font-bold text-[#2B2420] truncate">
                                  {ev.isHoliday ? '🎉 ' : '🎂 '} {ev.title}
                                </span>
                              </div>
                              <p className="text-[11px] font-black text-[#2B2420] mt-0.5">
                                Data: <span className="bg-[#F3E3B8] px-1.5 py-0.2 rounded text-[#2B2420]">{ev.dateShort}</span>
                              </p>
                            </div>
                            ...
                            <button
                              ...
                              className="px-2.5 py-1.5 rounded-xl bg-[#2B2420] text-white hover:bg-black font-extrabold text-[10px] flex items-center gap-1 shrink-0 active:scale-95 transition-transform cursor-pointer"
                              title="Mandar mensagem no WhatsApp sem ano"
                            >
                              <MessageCircle className="w-3.5 h-3.5 text-[#F5C6CE]" />
                              <span>Avisar</span>
                            </button>
                          </div>
                        ))}
      ...
      <div className="pt-4 border-t border-[#E8A0B0]/40 flex items-center justify-between flex-wrap gap-3 bg-white p-4 rounded-3xl border border-[#E8A0B0]/40 shadow-2xs">
        <div className="flex items-center gap-2.5">
          <div className="p-2 bg-emerald-100 text-emerald-700 rounded-xl">
            <MessageCircle className="w-5 h-5 text-emerald-600" />
          </div>
          <div>
            <h4 className="font-editorial-bold text-sm text-[#2B2420]">
              Envio via WhatsApp
            </h4>
            <p className="text-[11px] text-[#2B2420]/70 font-medium">
              Modo atual: <strong className="text-emerald-700 font-extrabold">{waPreference === 'business' ? 'WhatsApp Business' : waPreference === 'standard' ? 'WhatsApp Comum' : 'Perguntar Sempre'}</strong>
            </p>
          </div>
        </div>
        ...
        <button
          ...
          className="px-4 py-2 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs flex items-center gap-2 shadow-2xs transition-all active:scale-95 cursor-pointer"
      ...
      {waModalData && createPortal(
        <div className="fixed inset-0 z-[99999] bg-slate-900/80 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full shadow-2xl border-2 border-[#E8A0B0] overflow-hidden animate-fadeIn p-5 space-y-4 my-auto">
            <div className="flex items-center justify-between border-b border-[#F8F1E4] pb-2">
              <div className="flex items-center gap-2">
                <MessageCircle className="w-5 h-5 text-[#2B2420]" />
                <h3 className="font-brand font-black text-base text-[#2B2420]">
                  Enviar Mensagem WhatsApp
                </h3>
              </div>
              <button
                onClick={() => setWaModalData(null)}
                className="p-1 rounded-xl text-[#2B2420]/40 hover:text-[#2B2420]"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="space-y-2">
              <p className="text-xs text-[#2B2420]/80 font-medium">
                Cliente: <strong className="text-[#2B2420]">{waModalData.customer.name}</strong> ({waModalData.phone})
              </p>
              <div className="p-3 bg-[#F8F1E4] rounded-2xl border border-[#E8A0B0]/40 text-xs text-[#2B2420] font-medium whitespace-pre-wrap leading-relaxed max-h-48 overflow-y-auto">
                "{waModalData.message}"
              </div>
            </div>

            <div className="space-y-2">
              <span className="text-xs font-bold text-[#2B2420] block">
              ...
              <button
                ...
                className="w-full py-3 px-4 rounded-2xl bg-[#2B2420] hover:bg-black text-[#F5C6CE] font-bold text-xs shadow-md flex items-center justify-between transition-all active:scale-95"
              >
                <div className="flex items-center gap-2">
                  <span className="bg-[#F5C6CE] text-[#2B2420] p-1 rounded-lg font-black text-[10px]">
                    BUSINESS
                  </span>
                  <span>Abrir no WhatsApp Business</span>
                </div>
                <Sparkles className="w-4 h-4 text-[#F5C6CE]" />
              </button>
              ...
              <button
                ...
                className="w-full py-2.5 px-4 rounded-2xl bg-[#F8F1E4] hover:bg-[#F5C6CE]/30 text-[#2B2420] font-bold text-xs flex items-center justify-between transition-all active:scale-95 border border-[#E8A0B0]/40"
              >
                <span>Abrir no WhatsApp Comum / Padrão</span>
                <MessageCircle className="w-4 h-4 text-[#2B2420]/60" />
              </button>
              ...
              <button
                ...
                className="w-full py-2 px-4 rounded-2xl text-[#2B2420]/60 hover:text-[#2B2420] font-semibold text-xs text-center"
              >
            ...
            <div className="pt-3 border-t border-[#F8F1E4] flex items-center justify-between">
              <label className="flex items-center gap-2 text-[11px] text-[#2B2420]/80 font-bold cursor-pointer select-none">
                <input
                  type="checkbox"
                  ...
                  className="rounded border-[#E8A0B0] text-[#2B2420] focus:ring-[#2B2420]"
                />
