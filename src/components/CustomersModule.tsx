...
export const CustomersModule: React.FC = () => {
  ...
  const handleAddExtraEventField = () => {
    setAdditionalEvents((prev) => [
      ...prev,
      { id: Date.now().toString(), title: '', date: '', type: 'personal' },
    ]);
  };
      ...
      >
        {filteredCustomers.length === 0 ? (
          ...
          >
            ...
            >
              <Users className="w-[30px] h-[30px]" style={{ color: '#6E3F72' }} strokeWidth={2} />
            </div>
            <div className="font-serif-display" style={{ fontSize: '22px', color: '#3A2350' }}>
              ...
              {customers.length === 0 ? 'Nenhum cliente por aqui ainda' : 'Nenhum resultado encontrado'}
            </div>
            <div className="text-sm" style={{ color: '#7A6E80', maxWidth: '280px' }}>
              {customers.length === 0
                ? 'Cadastre sua primeira cliente e acompanhe os pedidos dela em um só lugar.'
