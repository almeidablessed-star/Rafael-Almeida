...
export const UserProfileModal: React.FC<UserProfileModalProps> = ({
  ...
  return createPortal(
    <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-slate-900/80 backdrop-blur-xs p-3 sm:p-4 overflow-y-auto">
      <div className="bg-white rounded-3xl max-w-lg w-full shadow-2xl overflow-hidden animate-fadeIn border-2 border-pink-200 my-auto max-h-[90vh] flex flex-col my-auto">
        {/* Header */}
        ...
        <form onSubmit={handleSave} className="p-5 overflow-y-auto space-y-4 flex-1">
          ...
          <div className="flex flex-col items-center justify-center space-y-2 p-3 bg-pink-50/70 rounded-2xl border border-pink-200">
            <div className="relative w-20 h-20 rounded-full bg-white border-2 border-[#FF5935] shadow-md flex items-center justify-center overflow-hidden">
              {photoUrl ? (
                <img
                  src={photoUrl}
                  alt="Perfil"
              ...
              <label className="absolute inset-0 bg-slate-900/40 opacity-0 hover:opacity-100 flex items-center justify-center text-white cursor-pointer transition-opacity">
            ...
            <div className="flex items-center gap-2">
              <label className="px-3 py-1 bg-pink-600 hover:bg-pink-700 text-white rounded-xl text-[11px] font-bold cursor-pointer shadow-2xs">
                📸 Escolher Foto/Logo
                <input
                  type="file"
                  accept="image/*"
          ...
          <div className="space-y-3">
            <div>
              ...
              <input
                ...
                className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-pink-500"
              />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-bold text-slate-800 mb-1 flex items-center gap-1">
                  <Phone className="w-3.5 h-3.5 text-emerald-600" /> Telefone / WhatsApp *
                </label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-xs font-bold text-slate-800 mb-1 flex items-center gap-1">
                  <MapPin className="w-3.5 h-3.5 text-amber-600" /> Endereço / Cidade *
                </label>
                <input
                  type="text"
                  required
          ...
          <div className="pt-3 border-t border-slate-100 flex items-center justify-end gap-2">
            ...
            <button
              type="submit"
              className="px-5 py-2 rounded-xl bg-pink-600 hover:bg-pink-700 text-white font-bold text-xs shadow-md flex items-center gap-1.5"
            >
              {savedSuccess ? (
                <>
