...
const CATEGORY_LABELS: Record<string, { label: string; icon: string; tagBg: string }> = {
  bolos: { label: 'Bolos & Massas', icon: '🎂', tagBg: 'bg-[#F5C6CE] text-[#2B2420]' },
  doces: { label: 'Doces & Sobremesas', icon: '🧁', tagBg: 'bg-[#D8CDEB] text-[#2B2420]' },
  salgados: { label: 'Salgados & Lanches', icon: '🥟', tagBg: 'bg-[#D6E4CC] text-[#2B2420]' },
  saudaveis: { label: 'Saudáveis & Fit', icon: '🥗', tagBg: 'bg-[#D6E4CC] text-[#2B2420]' },
  kids: { label: 'Kids Friendly', icon: '🧸', tagBg: 'bg-[#F3E3B8] text-[#2B2420]' },
};

const PALETTE_BG_CLASSES = [
  'bg-[#D6E4CC] border-[#2B2420]/10', // Verde Sálvia
  'bg-[#D8CDEB] border-[#E8A0B0]/40', // Lavanda
  'bg-[#F5C6CE] border-[#E8A0B0]',    // Rosa Soft
  'bg-[#F3E3B8] border-[#E8A0B0]/40', // Amarelo Manteiga
];
...
export const FichasTecnicasModule: React.FC<FichasTecnicasModuleProps> = ({
  ...
  return (
    <div className="space-y-5 pb-12 animate-fadeIn">
      ...
      <div className="bg-[#D8CDEB] rounded-[32px] p-5 sm:p-6 text-[#2B2420] shadow-2xs border border-[#E8A0B0]/40 relative overflow-hidden flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
        ...
        <button
          ...
          className="px-5 py-3 rounded-2xl bg-[#2B2420] hover:bg-black text-[#F5C6CE] font-bold text-xs shadow-md active:scale-95 transition-all flex items-center gap-2 cursor-pointer shrink-0"
        >
      ...
      {launchSuccessMsg && (
        <div className="bg-emerald-100 border border-emerald-300 text-emerald-900 text-xs p-3.5 rounded-2xl flex items-center gap-2.5 animate-fadeIn shadow-2xs">
      ...
      <div className="flex items-center gap-1.5 bg-[#F8F1E4] p-1.5 rounded-[24px] border border-[#E8A0B0]/40 overflow-x-auto no-scrollbar shadow-2xs">
        <button
          onClick={() => setSelectedCategory('bolos')}
          className={`px-3.5 py-2.5 rounded-2xl text-xs font-bold transition-all flex items-center gap-1.5 whitespace-nowrap cursor-pointer ${
            selectedCategory === 'bolos'
              ? 'bg-[#2B2420] text-[#F5C6CE] shadow-sm scale-102'
              : 'bg-white/80 text-[#2B2420] hover:bg-white'
          }`}
        >
          <span>🎂 Bolos</span>
        </button>
      ...
      {/* Editor Modal / Form */}
      {isCreating && (
        <form onSubmit={handleSaveFicha} className="bg-white rounded-[32px] p-5 sm:p-6 border-2 border-[#E8A0B0] shadow-xl space-y-4 animate-slideUp">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <h3 className="font-brand font-black text-lg text-[#2B2420] flex items-center gap-2">
              <Sparkles className="w-5 h-5 text-[#E8A0B0]" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
            <div>
              ...
              <input
                ...
                className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-xs font-bold text-[#2B2420] focus:outline-none focus:ring-2 focus:ring-[#E8A0B0]"
              />
            </div>
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-xs font-extrabold text-[#2B2420] focus:outline-none focus:ring-2 focus:ring-[#E8A0B0] bg-white"
              >
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                ...
                <select
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-xs font-extrabold text-[#2B2420] focus:outline-none focus:ring-2 focus:ring-[#E8A0B0] bg-white cursor-pointer"
                >
            ...
            <div>
              ...
              <div className="flex items-center gap-2.5">
                {imageUrl ? (
                  <div className="relative w-11 h-11 rounded-xl border-2 border-[#E8A0B0] overflow-hidden shrink-0 shadow-2xs group">
                    <img src={imageUrl} alt="Preview" className="w-full h-full object-cover" />
                    <button
                      type="button"
                      onClick={() => setImageUrl('')}
                      className="absolute top-0 right-0 bg-rose-600 text-white p-0.5 rounded-bl text-[9px] font-black hover:bg-rose-700 transition cursor-pointer"
                      title="Remover Foto"
                    >
                      <X className="w-3 h-3" />
                    </button>
                  ...
                  <div className="w-11 h-11 rounded-xl bg-[#F8F1E4] border border-[#E8A0B0]/50 flex items-center justify-center shrink-0 text-[#2B2420]/40">
                    <ImageIcon className="w-5 h-5" />
                  </div>
                )}

                <label className="flex-1 px-4 py-2.5 bg-[#2B2420] hover:bg-black text-[#F5C6CE] rounded-xl text-xs font-bold cursor-pointer text-center shadow-2xs transition-all active:scale-95 flex items-center justify-center gap-2">
                  <Upload className="w-4 h-4 text-[#F5C6CE]" />
          ...
          <div className="bg-[#F8F1E4] p-4 rounded-2xl border border-[#E8A0B0]/40 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-extrabold uppercase text-[#2B2420]">
                Ingredientes ({ingredients.length})
              </span>
              <button
                type="button"
                onClick={handleAddIngredient}
                className="text-xs font-bold text-[#2B2420] hover:underline flex items-center gap-1 cursor-pointer"
              >
                <PlusCircle className="w-4 h-4 text-[#E8A0B0]" />
                <span>Adicionar</span>
              </button>
            </div>
            ...
            <div className="space-y-2">
              {ingredients.map((ing) => (
                <div key={ing.id} className="bg-white p-2.5 rounded-xl border border-[#E8A0B0]/30 grid grid-cols-12 gap-2 items-center text-xs">
                  ...
                  <div className="col-span-3 flex items-center gap-1">
                    <input
                      ...
                      className="w-full px-1.5 py-1 border border-slate-300 rounded-lg text-xs font-bold text-center"
                    />
                    <select
                      value={ing.unit}
                      onChange={(e) => handleUpdateIngredient(ing.id, 'unit', e.target.value)}
                      className="px-1 py-1 border border-slate-300 rounded-lg text-[10px] font-bold"
                    >
                  ...
                  <div className="col-span-4 text-right">
                    <span className="text-[10px] text-slate-500 block">Custo Gasto</span>
                    <span className="font-black text-xs text-[#2B2420]">
            ...
            <div className="pt-2 border-t border-[#E8A0B0]/30 flex items-center justify-between text-xs font-bold text-[#2B2420]">
              <span>Total Reposição (Ingredientes):</span>
              <span className="font-extrabold text-sm text-[#2B2420]">
          ...
          <div className="grid grid-cols-3 gap-2.5 text-xs">
            <div>
              <label className="block text-[11px] font-bold text-[#2B2420] mb-1">
                Mão de Obra ($)
              </label>
              <input
                type="text"
                ...
                className="w-full px-2.5 py-2 border border-slate-300 rounded-xl font-bold text-[#2B2420]"
              />
            </div>
            <div>
              <label className="block text-[11px] font-bold text-[#2B2420] mb-1">
          ...
          <div className="bg-[#2B2420] p-4 rounded-2xl text-white flex items-center justify-between">
            <div>
              <span className="text-[10px] font-bold uppercase tracking-wider text-[#F5C6CE] block">
                Sugestão de Preço de Venda
              </span>
              <span className="text-2xl font-black text-[#F8F1E4]">
                {formatCurrency(calculatedSuggestedPrice)}
              </span>
            </div>
            <div className="text-right text-[11px] text-[#F8F1E4]/80 font-medium">
          ...
          <div className="flex items-center justify-end gap-2 pt-2">
            ...
            <button
              type="submit"
              className="px-5 py-2.5 rounded-xl bg-[#2B2420] hover:bg-black text-[#F5C6CE] font-bold text-xs shadow-md flex items-center gap-1.5 cursor-pointer"
            >
      ...
      <div className="space-y-4">
        {filteredFichas.length === 0 ? (
          <div className="p-8 rounded-[32px] bg-white border border-dashed border-[#E8A0B0]/50 text-center space-y-3">
            <BookOpen className="w-10 h-10 text-[#2B2420]/40 mx-auto" />
            <p className="text-xs text-[#2B2420]/70 font-semibold">
              Nenhuma ficha técnica cadastrada neste setor.
            </p>
            <button
              onClick={handleOpenAdd}
              className="px-4 py-2 rounded-full bg-[#2B2420] text-[#F5C6CE] text-xs font-bold transition active:scale-95 cursor-pointer"
            >
          ...
          filteredFichas.map((ficha, index) => {
            ...
            return (
              <div
                key={ficha.id}
                className={`${bgClass} rounded-[32px] p-5 sm:p-6 shadow-2xs transition-all hover:shadow-md relative overflow-hidden`}
              >
                <div className="grid grid-cols-1 md:grid-cols-12 gap-5 items-center">
                  ...
                  <div className="md:col-span-7 space-y-3.5">
                    ...
                    <h3 className="font-editorial-bold text-2xl sm:text-3xl text-[#2B2420] leading-snug">
                      {ficha.name}
                    </h3>

                    {/* Yield Info */}
                    <p className="text-xs text-[#2B2420]/80 font-semibold flex items-center gap-1.5">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5 text-center text-[10px] font-semibold pt-1">
                      <div className="bg-white/80 px-2 py-1.5 rounded-xl border border-black/5 flex flex-col justify-center items-center">
                        <span className="text-[#2B2420]/70 text-[9px] font-bold leading-tight uppercase tracking-wider">Reposição</span>
                        <strong className="text-[#2B2420] text-[11px] font-extrabold mt-0.5">{formatCurrency(repoTotal)}</strong>
                      </div>
                      <div className="bg-white/80 px-2 py-1.5 rounded-xl border border-black/5 flex flex-col justify-center items-center">
                        <span className="text-[#2B2420]/70 text-[9px] font-bold leading-tight uppercase tracking-wider">Mão de Obra</span>
                        <strong className="text-[#2B2420] text-[11px] font-extrabold mt-0.5">{formatCurrency(ficha.maoDeObraCost)}</strong>
                      </div>
                      <div className="bg-white/80 px-2 py-1.5 rounded-xl border border-black/5 flex flex-col justify-center items-center">
                        <span className="text-[#2B2420]/70 text-[9px] font-bold leading-tight uppercase tracking-wider">Custos Op.</span>
                        <strong className="text-[#2B2420] text-[11px] font-extrabold mt-0.5">
                          {formatCurrency((ficha.custoCost || 0) + (ficha.investimentoCost || 0))}
                        </strong>
                      </div>
                      <div className="bg-[#2B2420] px-2 py-1.5 rounded-xl text-white flex flex-col justify-center items-center">
                        <span className="text-[#F5C6CE] text-[9px] font-extrabold leading-tight uppercase tracking-wider">Sugestão</span>
                        <strong className="text-[#F8F1E4] text-[11px] font-black mt-0.5">
                    ...
                    <div className="flex items-center gap-2 pt-1 flex-wrap">
                      <button
                        ...
                        className="px-3.5 py-1.5 rounded-full bg-white/80 hover:bg-white text-[#2B2420] font-bold text-xs transition flex items-center gap-1 cursor-pointer shadow-2xs"
                      >
                    ...
                    {isExpanded && (
                      <div className="bg-white/90 p-3.5 rounded-2xl border border-black/5 text-xs space-y-2 animate-fadeIn mt-2">
                        <span className="text-[10px] font-black uppercase text-[#2B2420]/60 tracking-wider block">
                          Ingredientes & Custos de Produção:
                        </span>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1 text-[11px] font-medium text-[#2B2420]">
                          {ingList.map((ing) => (
                            <div key={ing.id} className="flex items-center justify-between border-b border-black/5 pb-1">
                              <span>
                                {ing.quantity} {ing.unit} de {ing.name}
                              </span>
                              <strong className="text-[#2B2420]">
                  ...
                  <div className="md:col-span-5 flex flex-col gap-2.5">
                    ...
                    <div className="flex items-center justify-center sm:justify-end gap-1.5 pt-0.5 flex-wrap">
                      <button
                        ...
                        className="px-3 py-1.5 rounded-xl bg-white/80 hover:bg-white text-[#2B2420] text-xs font-bold transition shadow-2xs flex items-center gap-1 cursor-pointer"
                        title="Editar Ficha"
                      >
                        <Edit3 className="w-3.5 h-3.5 text-[#2B2420]" />
                        <span>Editar</span>
                      </button>
                      ...
                      >
                        <Copy className="w-3.5 h-3.5 text-[#2B2420]" />
                        <span>Duplicar</span>
                      </button>

                      <button
                        onClick={() => handleDelete(ficha.id)}
                        className="px-3 py-1.5 rounded-xl bg-white/80 hover:bg-rose-100 text-rose-700 text-xs font-bold transition shadow-2xs flex items-center gap-1 cursor-pointer"
                        title="Excluir Ficha"
                      >
                        <Trash2 className="w-3.5 h-3.5" />
                        <span>Excluir</span>
